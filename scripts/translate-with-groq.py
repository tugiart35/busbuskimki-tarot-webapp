#!/usr/bin/env python3
"""
🚀 GROQ AI ile Tarot Çevirileri
- 100x daha hızlı (Google Translate'e göre)
- Otomatik Latin alfabesi (Sırpça)
- Progress bar + ETA
- File lock + Duplicate önleme
- Batch çeviri (5 kart aynı anda)
"""
...
# ═══════════════════════════════════════════════════════════
SPREAD_KEY = 'money'
POSITION_NUM = 5  # ← DEĞİŞTİR
# ═══════════════════════════════════════════════════════════

# Groq API Configuration
GROQ_API_KEY = os.getenv('GROQ_API_KEY') or "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"
GROQ_API_URL = "https://api.groq.com/openai/v1/chat/completions"
GROQ_MODEL = "llama-3.3-70b-versatile"  # Yeni model (2025)

LOCK_FILE = f'/tmp/tarot-i18n-{SPREAD_KEY}-pos{POSITION_NUM}.lock'
BATCH_SIZE = 1  # Rate limit'i düşürmek için 1 kart (önceden 3)
RETRY_MAX = 3  # Kaç kez tekrar dene
RETRY_DELAY = 60  # Rate limit hatası alınca 60 saniye bekle
REQUEST_DELAY = 5  # Her istek arasında 2 saniye bekle
...
{lang_name} Output (same JSON structure, same length):"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # JSON extract et
            # Bazen ```json ... ``` ile sarmalanmış olabilir
            ...
            # Key düzeltmeleri (Groq bazen yanlış key kullanabiliyor)
            ...
            # Sırpça için ekstra Latin dönüşümü (güvenlik)
            if target_lang == 'sr':
...
def main():
    ...
    print("=" * 80)
    print(f"🚀 GROQ AI - {SPREAD_KEY.upper()} POSITION-{POSITION_NUM} ÇEVİRİ")
    print("=" * 80)
    ...
    print(f"📦 Batch size: {BATCH_SIZE} kart")
    ...
    print(f"\n🃏 Groq AI çevirileri başlıyor...")
    print(f"⏱️  Tahmini süre: ~{int(total_cards * 0.15)} - {int(total_cards * 0.2)} dakika")
    print(f"⚡ Batch size: {BATCH_SIZE} kart/istek")
    print(f"⏱️  Request delay: {REQUEST_DELAY} saniye (rate limit önleme)")
    print(f"🔄 Retry: {RETRY_MAX} deneme, {RETRY_DELAY}sn backoff")
    print("=" * 80)
    print(f"\n{'Kart':<20} {'EN':<8} {'SR':<12} {'Süre':<10} {'ETA'}")
    print("-" * 65)
    ...
    for idx, (card_key, card_data) in enumerate(card_list):
        ...
        # İngilizce çeviri
        en_status = "⏳"
        en_translated = translate_with_groq(to_translate, 'en')
        if en_translated:
            if card_key not in en_data[SPREAD_KEY]['meanings']:
                en_data[SPREAD_KEY]['meanings'][card_key] = {}
            en_data[SPREAD_KEY]['meanings'][card_key][f'position{POSITION_NUM}'] = en_translated
            en_status = "✅"
            successful_en += 1
        else:
            en_status = "❌"
            failed_cards.append(f"{card_key}/EN")
        ...
        if sr_translated:
            ...
            sr_status = "❌"
            failed_cards.append(f"{card_key}/SR")
        
        card_time = time.time() - card_start
        
        # Detaylı ilerleme satırı
        progress_pct = int((current / total_cards) * 100)
        bar = '█' * int(progress_pct / 2) + '░' * (50 - int(progress_pct / 2))
        
        print(f"{card_key:<20} {en_status:<8} {sr_status:<12} {int(card_time)}s{'':<6} ETA: {eta}")
        
        # Her 10 kartta özet
        if current % 10 == 0:
            print(f"\n💾 Ara özet [{current}/{total_cards}]:")
            print(f"  ✅ Başarılı EN: {successful_en}/{current}")
    ...
    try:
        ...
        for card_key, card_data in en_data[SPREAD_KEY]['meanings'].items():
            ...
            # Duplicate önleme
            pos_key = f'position{POSITION_NUM}'
            if pos_key in existing_en[SPREAD_KEY]['meanings'][card_key]:
                print(f"  ⚠️ EN {card_key} position{POSITION_NUM} zaten var, atlanıyor")
            else:
                existing_en[SPREAD_KEY]['meanings'][card_key][pos_key] = card_data[pos_key]
    ...
    print(f"\n📊 SONUÇLAR:")
    print(f"  ✅ Başarılı EN: {successful_en}/{total_cards} ({int(successful_en/total_cards*100)}%)")
    print(f"  ✅ Başarılı SR: {successful_sr}/{total_cards} ({int(successful_sr/total_cards*100)}%)")
    print(f"  ❌ Başarısız: {len(failed_cards)}")
    if failed_cards:
    ...
    print(f"  Google Translate: ~12-15 sn/kart")
    print(f"  Hız kazancı: ~{12 / (total_time/total_cards):.1f}x daha hızlı!")
    print(f"\n📁 Dosyalar:")
    print(f"  - messages/en.json ({successful_en} kart)")
